...
const usePersistentData = () => {
  const [data, setData] = useState<AppData>(() => {
    ...
    try {
      ...
      if (parsed.tokensAvailable === undefined) {
        const tier1Claims = parsed.claims?.filter((claim) => claim.tier === 1) ?? [];
        merged.tokensAvailable = Math.max(
          0,
          (parsed.blocks?.length ?? 0) - tier1Claims.length
        );
      }
      return merged;
    } catch {
...
const App = () => {
  ...
  const tier1Tokens = Math.max(0, data.tokensAvailable);
  ...
  const handleLogBlock = () => {
    ...
    setData((prev) => ({
      ...
      tokensAvailable:
        prev.settings.tokenMode === "auto"
          ? prev.tokensAvailable + 1
          : prev.tokensAvailable,
      pendingBlock: null,
    }));
    setPendingNote("");
    setToast(
      data.settings.tokenMode === "auto"
        ? "Block logged. Tier 1 token earned."
        : "Block logged."
    );
  ...
  const handleClaim = () => {
    ...
    if (claimDraft.tier === 1 && data.tokensAvailable <= 0) {
    ...
    setData((prev) => ({
      ...
      tokensAvailable:
        claimDraft.tier === 1
          ? Math.max(0, prev.tokensAvailable - 1)
          : prev.tokensAvailable,
    }));
    setClaimDraft(null);
    setClaimNote("");
  ...
  const handleGrantToken = () => {
    setData((prev) => ({
      ...prev,
      tokensAvailable: prev.tokensAvailable + 1,
    }));
        ...
        {activeTab === "rewards" && (
          <section className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
            <div className="grid gap-6">
              ...
              />
              {data.settings.tokenMode === "manual" && (
                <div className="rounded-2xl border border-ink-100 bg-white p-4 text-sm shadow-sm">
                  <p className="font-semibold text-ink-900">
                    Manual token mode
                  </p>
                  <p className="mt-1 text-ink-500">
                    Click to grant yourself a Tier 1 token when you finish a
                    focused block.
                  </p>
                  <button
                    type="button"
            ...
            <div className="rounded-2xl border border-ink-100 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 grid gap-4 text-sm text-ink-700">
                ...
                <label className="flex flex-col gap-2">
                  Tier 1 token mode
                  <select
                    value={data.settings.tokenMode}
                    onChange={(event) =>
                      setData((prev) => ({
                        ...prev,
                        settings: {
                          ...prev.settings,
                          tokenMode: event.target
                            .value as AppData["settings"]["tokenMode"],
                        },
                      }))
                    }
                    className="rounded-xl border border-ink-100 px-3 py-2 focus:border-ink-300 focus:outline-none"
                  >
                    <option value="auto">Auto (1 token per completed block)</option>
                    <option value="manual">Manual (click to grant tokens)</option>
                  </select>
                </label>
                <div>
